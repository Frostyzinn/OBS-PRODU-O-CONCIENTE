...
router.post('/',auth,requireRole('admin','production'),ah(async(req,res)=>{
 ...
 if([b.minStock??0,b.currentStock??0,b.unitCost??0].some(v=>!Number.isFinite(Number(v))||Number(v)<0))return res.status(400).json({error:'Estoque e custo devem ser números não negativos.'});
...
router.post('/:id/movements',auth,requireRole('admin','production'),ah(async(req,res)=>{
 ...
 const rm=await tx.get('SELECT id,current_stock FROM raw_materials WHERE id=? AND company_id=? FOR UPDATE',[id,cid]);
 if(!rm)throw Object.assign(new Error('Matéria-prima não encontrada'),{status:404});
 const balance=Number(rm.current_stock)+(mtype==='out'?-qty:qty);
 if(balance<0)throw Object.assign(new Error('Estoque insuficiente.'),{status:400});
 await tx.run('INSERT INTO stock_movements(company_id,raw_material_id,user_id,type,quantity,balance_after,reference,note) VALUES(?,?,?,?,?,?,?,?)',[cid,id,req.user.id,mtype,qty,balance,String(req.body?.reference||''),String(req.body?.note||'')]);
 await tx.run('UPDATE raw_materials SET current_stock=? WHERE id=? AND company_id=?',[balance,id,cid]);return balance;
 });
